...
exports.updateBgDatabase = (req, res, next) => {
        ...
        let xml = XML2JS.parseString(response.data, (err, result) => {
          ...
          if (result.items.$.totalitems !== "0") {
            ...
            result.items.item.forEach(bgItem => {
              ...
              boardgame.bggObjId = bgItem.$.objectid;
            ...
            Boardgame.insertMany(boardgames, function(err, docs) {
              if (err) console.log(err);
              else console.log("all insert");
            });
            req.body.bgList = result;
          }
        });
